...
fileItem = initRequest(request);	
		...
		working=getFieldValue(fileItem, "working");
		intlike=getFieldValue(fileItem, "aoi");
		...
		try 
		{
			...
			ps=con.prepareStatement("insert into mentor_info (name,lastname,age,gender,email,mobno,designation,experience,working,intlike,username,password) values(?,?,?,?,?,?,?,?,?,?,?,?)");
			...
			if (result > 0)
			{
				...
				out.println("<a href=mentorlogin.jsp><h2>Login Here</h2></a>");
				out.println("</center>");
				out.println("</body>");
				out.println("</html>");
